exports.config = {
  ...
  // path for all Stencil build assets. By default the loader
  // script will automatically figure out the relative path to
  // load your components, but uses publicPath as a fallback.
  // It's recommended to have it start with /. Note that this
  // only sets the base path the browser requests, but this does
  // not set where files are saved during build. To change where
  // files are saved at build time, use the buildDir config.
  publicPath: "/js/scripts/webcomponents/zapstencilcomponents/dist",
  ...
  // copy
  // The copy config is an array of objects that define any files or folders that
  // you would like to get copied over to your buildDir when a build is performed.Each
  //  object in the array must include a src property which can be either an absolute path,
  // a relative path or a glob pattern.You can also provide the optional dest property which
  // can be either an absolute path or a path relative to your buildDir.
